...
class Music_model(tf.keras.Model):
    ...
    def call(self, input, training=False):
        task_feature, answers = input
        # cls_out = self.left_NN(task_feature, training)
        sample_task_feature = self.right_NN(task_feature, training)
        crowd_bias  = self.worker_NN(sample_task_feature, answers)
        return crowd_bias, sample_task_feature
    ...
    def loss_fuction(self, task_feature, crowd_bias, answers, gamma=0.1):
        # CE_loss
        EC_loss = self.CE_loss(crowd_bias, answers)
        ...
        total_loss = tf.reduce_sum(EC_loss) #+ kl_loss  #+ poster
...
def run_Music():
    ...
    for epoch in range(1000):
        ...
        for step in range(steps):
            ...
            with tf.GradientTape() as tape:
                crowd_bias, sample_task_feature = trainer([task_feature, answers], training=True)

                loss = trainer.loss_fuction(task_feature, crowd_bias, answers[:, -1])

                vars = tape.watched_variables()
                grads = tape.gradient(loss, vars)
                optimizer.apply_gradients(zip(grads, vars))

        crowd_bias, sample_task_feature = trainer([task_feature, answers], training=False)
