...
class LableMe_model(tf.keras.Model):
    ...
    def kl_Qwtheta_Pw(self, w, mu, sigma):
        Qwtheta = tf.math.log(self.gaussian_distribution_density(w, mu, sigma))  # poster
        Pw = tf.math.log(self.gaussian_distribution_density(w, 0.0, 0.01))  # prior
...
class Music_model(tf.keras.Model):
    def __init__(self, task_num, feature_size, worker_num, class_num, hidden_size, answer_num, a=0.01, l=0.5, *args, **kwargs):
        ...
        self.worker_matrix = tf.Variable(
            tf.random.normal((self.worker_num, self.class_num, self.class_num), mean=0, stddev=0.01),
            name='worker_mu'
        )
    ...
    def worker_NN(self, task_feature, answers):
        ...
        masked_task_feature = tf.expand_dims(tf.gather(task_feature, row), axis=1)
        self.masked_worker_matrix = tf.gather(self.worker_matrix, col - self.task_num)


        crowd_bias = tf.squeeze(tf.matmul(masked_task_feature, self.masked_worker_matrix), axis=1) # tf.nn.sigmoid(masked_worker_rho) tf.math.log(1+tf.math.exp(masked_worker_rho))
    ...
    def loss_fuction(self, task_feature, recons_task_feature, crowd_bias, answers, task_mu, task_log_sigma, gamma=0.1):
        ...
        # return tf.reduce_sum(EC_loss), mig_loss, kl_loss
# class Music_model(tf.keras.Model):
#     def __init__(self, task_num, feature_size, worker_num, class_num, hidden_size, answer_num, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         self.task_num = task_num
#         self.feature_size = feature_size
#         self.worker_num = worker_num
...
#         # self.worker_feature = tf.one_hot(indices=range(self.worker_num), depth=self.worker_num)
#
#         self.worker_mu_fc = tf.keras.layers.Dense(self.worker_num, activation=None, name='worker_mu_fc')
#         self.worker_sigma_fc = tf.keras.layers.Dense(self.worker_num, activation=None, name='worker_sigma_fc')
#
#         # self.flatten = tf.keras.layers.Flatten()
#         # self.left_fc1 = tf.keras.layers.Dense(self.hidden_size, activation='relu', name='task_lfc_1')
#         # self.left_fc2 = tf.keras.layers.Dense(self.class_num, name='task_lfc_2')
...
#         self.right_bn = tf.keras.layers.BatchNormalization(center=False, scale=False)
#         self.right_bn1 = tf.keras.layers.BatchNormalization(center=False, scale=False)
#         self.right_bn2 = tf.keras.layers.BatchNormalization(center=False, scale=False)
#         self.fc_task_mu = tf.keras.layers.Dense(self.class_num, activation=None, name='task_rfc_mu')
#         self.fc_task_log_sigma = tf.keras.layers.Dense(self.class_num, activation=None, name='task_rfc_log_sigma')
#
#         self.mu_bn = tf.keras.layers.BatchNormalization(center=False, scale=False)
...
#     def sample(self, mean, log_std):
#         std = tf.math.exp(log_std)
#         eps = tf.random.normal(shape=(self.task_num, self.class_num), mean=0, stddev=0.1)
#         return mean + eps * std
#
#     # def left_NN(self, task_feature, training=None):
#     #
...
#
#     def worker_NN(self, task_feature, answers):
#         self.worker_mu = tf.transpose(self.worker_mu_fc(tf.transpose(self.task_mu)))
#         self.worker_sigma = tf.math.exp(tf.transpose(self.worker_sigma_fc(tf.transpose(self.task_log_sigma))))
#
#         task_ids = answers[:, 0]
#         worker_ids = answers[:, 1]
#         label_ids = answers[:, 2]
...
#         self.masked_task_sigma = tf.math.exp(tf.gather(self.task_log_sigma, row))
#         self.masked_worker_mu = tf.gather(self.worker_mu, col - self.task_num)
#         self.masked_worker_sigma = tf.gather(self.worker_sigma, col - self.task_num)
#
#         # mu_mn = self.masked_task_mu + self.masked_worker_mu
#         # sigma2_mn = self.masked_task_sigma ** 2 + self.masked_worker_sigma ** 2
#         sigma2_mn = 1 / (1 / (self.masked_task_sigma) ** 2 + 1 / (self.masked_worker_sigma) ** 2)
#         mu_mn = sigma2_mn * (self.masked_task_mu / (self.masked_task_sigma) ** 2 + self.masked_worker_mu / (
#             self.masked_worker_sigma) ** 2)
#         s_mn = self.gaussian_distribution_density(
#             self.masked_task_mu - self.masked_worker_mu, 0,
#             tf.sqrt((self.masked_task_sigma) ** 2 + (self.masked_worker_sigma) ** 2)
#         )
#         crowd_bias = mu_mn + tf.math.log(s_mn/tf.sqrt(sigma2_mn))
#         # s = tf.range(10)
#         # crowd_bias = masked_task_feature * tf.nn.softplus(self.masked_worker_rho) + self.masked_worker_mu # tf.nn.sigmoid(masked_worker_rho) tf.math.log(1+tf.math.exp(masked_worker_rho))
...
#         return crowd_bias, sample_task_feature, recons_task_feature, task_mu, task_log_sigma
#
#     def kl_Qwtheta_Pw(self):
#         return tf.reduce_mean(-tf.reduce_sum(tf.math.log(self.worker_sigma), axis=1) + tf.reduce_sum(
#             tf.square(self.worker_mu) + tf.square(self.worker_sigma), axis=1) / (2*self.pripr_var))
#
...
#             tf.reduce_sum(1 + 2 * task_log_sigma - tf.square(task_mu) - tf.square(tf.exp(task_log_sigma)),
#                           axis=1) * 0.5)
#
#     def loss_fuction(self, task_feature, recons_task_feature, crowd_bias, answers, task_mu, task_log_sigma, gamma=0.1):
#         # # 定义模型
...
#         #                             tf.math.reduce_sum(tf.math.exp(2 * self.task_log_sigma), axis=0))
#         poster_prior = self.kl_Qwtheta_Pw()
#         # poster = self.kl_Qwtheta_Pw(self.worker_mu,
#         #                             mu_,
#         #                             sigma_2) \
#         #          + \
...
#         EC_loss = self.CE_loss(crowd_bias, answers)
#
#         total_loss = (mig_loss + kl_loss) + tf.reduce_mean(EC_loss) + poster_prior
#
#         # loss 来自 KL，与MIG相反数
#         return total_loss
#         # return tf.reduce_sum(EC_loss), mig_loss, kl_loss
...
#             tf.sqrt((self.masked_task_sigma) ** 2 + (self.masked_worker_sigma) ** 2)
#         )
#         crowd_bias = mu_mn + tf.math.log(s_mn / sigma2_mn)
#         # s = tf.range(10)
#         # crowd_bias = masked_task_feature * tf.nn.softplus(self.masked_worker_rho) + self.masked_worker_mu # tf.nn.sigmoid(masked_worker_rho) tf.math.log(1+tf.math.exp(masked_worker_rho))
#         # crowd_bias = self.gaussian_distribution_density(masked_task_feature, self.masked_worker_mu, tf.nn.softplus(self.masked_worker_rho))
#         # self.poster_prior = self.kl_Qwtheta_Pw(worker_mu_kernel, self.workerMu_mu, self.workerMu_rho) \
...
#
#     def KL_loss(self, task_mu, task_log_sigma):
#         return -tf.reduce_mean(tf.reduce_sum(1 + 2 * task_log_sigma - tf.square(task_mu) - tf.square(tf.exp(task_log_sigma)), axis=1) * 0.5)
#
#     def loss_fuction(self, task_feature, recons_task_feature, crowd_bias, answers, task_mu, task_log_sigma, gamma=0.1):
#         # # 定义模型
#         # gmm = GMM(n_components=self.class_num)
...
class MacroF1:
    def __init__(self, sample_size, nums_label):
        self.sample_size = sample_size
        self.nums_label = nums_label

    def macro_f1_score(self, y_true, y_pred):
        f1_scores = 0
        for i in range(self.nums_label):
            tmp_true = [0] * self.sample_size
            tmp_pred = [0] * self.sample_size

            for j in range(self.sample_size):
                tmp_true[j]=1 if y_true[j] == i else 0
                tmp_pred[j]=1 if y_pred[j] == i else 0

            f1_test = F1metrics(tmp_true, tmp_pred)
            _, (tp,fp,tn,fn) = f1_test.f1_score()
            macro_precision = tp/(tp + fp) if (tp + fp) > 0 else 0
            macro_recall = tp/(tp + fn) if (tp + fn) > 0 else 0
            if macro_precision + macro_recall == 0:
                f1_scores += 0
            else:
                f1_scores += (2 * macro_precision * macro_recall)/(macro_precision + macro_recall)
        f1_scores = f1_scores/self.nums_label
...
def run_Music(inf='SI', sample_num=100, a=0.01, l=0.5):
    ...
    for epoch in tqdm(range(1000)):
        ...
        print('.................')
    # np.savetxt('AccLine_music_{}_{}.csv'.format(str(hidden_size), str(time)), np.array(acc_cache))
    # np.savetxt('MacroLine_music_{}_{}.csv'.format(str(hidden_size), str(time)), np.array(f1_cache))
...
best_acc, best_macro, is_nan = run_Music(inf) #OK
