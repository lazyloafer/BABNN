...
class Music_model(tf.keras.Model):
    def __init__(self, task_num, feature_size, worker_num, class_num, answer_num=None, *args, **kwargs):
        ...
        self.right_fc2 = tf.keras.layers.Dense(self.hidden_size, activation='relu')
        self.right_fc3 = tf.keras.layers.Dense(self.class_num, activation=None)
        self.right_fc4 = tf.keras.layers.Dense(self.feature_size, activation=None)
    ...
    def call(self, input, training=False):
        task_feature, answers = input
        # cls_out = self.left_NN(task_feature, training)
        sample_task_feature, recons_task_feature = self.right_NN(task_feature, training)
        crowd_bias  = self.worker_NN(sample_task_feature, answers)
        return crowd_bias, sample_task_feature, recons_task_feature
    ...
    def loss_fuction(self, task_feature, recons_task_feature, crowd_bias, answers, gamma=0.1):
        # CE_loss
        EC_loss = self.CE_loss(crowd_bias, answers)
        ...
        total_loss = mse_loss + tf.reduce_sum(EC_loss) #+ kl_loss  #+ poster
...
class SP_model(tf.keras.Model):
    ...
    def loss_fuction(self, task_feature, recons_task_feature, crowd_bias, answers, gamma=0.1):
        ...
        total_loss = mig_loss + tf.reduce_sum(EC_loss) # + kl_loss + poster
...
def run_Music():
    ...
    for epoch in range(1000):
        ...
        for step in range(steps):
            ...
            with tf.GradientTape() as tape:
                crowd_bias, sample_task_feature, recons_task_feature = trainer([task_feature, answers], training=True)

                loss = trainer.loss_fuction(task_feature, recons_task_feature, crowd_bias, answers[:, -1])

                vars = tape.watched_variables()
                grads = tape.gradient(loss, vars)
                optimizer.apply_gradients(zip(grads, vars))

        crowd_bias, sample_task_feature, recons_task_feature = trainer([task_feature, answers], training=False)
